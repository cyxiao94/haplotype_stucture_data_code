...
run_slim_simulation(){
  ...
  do
    ...
    do
      ...
      slim -s ${seed} \
         -d "vcffile='/home/vetlinux04/Changyi/04Dsim_SA_flucVScons/06slim_2025/01simulation_10runs_chr2L/01generate_founder_population/04vcffile_LE/run${runs}_n${num_sample}_${num_target}loci_s10_${pop}.vcf'" \
         -d "popsize=${popsize}" \
         -d "strength=${strength}" \
