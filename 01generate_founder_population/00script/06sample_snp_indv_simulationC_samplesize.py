...
np.savetxt(f"07sample_index_simulationC/run{output_prefix}_n{num_sample}_{num_target}loci_snp_index.txt", np.array(sampled_target), fmt="%s",header="#SNP index",comments=f"#Seed {seed}\n#index showed the (transformed) postion of the SNP\n")


# %%
#sample individuals
fo=open(f"07sample_index_simulationC/run{output_prefix}_n{num_sample}_{num_target}loci_sample_index.txt","w")
fo.write(f"#Seed {seed}\n")
fo.write(f"#Run {output_prefix}\n")

for num_sample in [num_sample]:
